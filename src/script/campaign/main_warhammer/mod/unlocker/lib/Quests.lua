local log = require('unlocker/log')('unlocker:quests');
local TAUROX_FACTION = require('unlocker/constants').TAUROX_FACTION;
...
function Quests:addTauroxQuestBattleListener()
  ...
  if not cm:get_saved_value('unlocker_taurox_bst_taurox_battle_quest') then
    core:add_listener(
      'Taurox_Quest_Battle',
      'FactionTurnStart',
      function(context)
      ...
      function()
        log('Trigger mission unlocker_taurox_bst_taurox_dual_cleaver');
        cm:trigger_mission(TAUROX_FACTION, 'unlocker_taurox_bst_taurox_dual_cleaver', true);
        cm:set_saved_value('unlocker_taurox_bst_taurox_battle_quest', true);
      end,
      false
    );
  end;
...
function Quests:setupRankupListerners()
  ...
  local set_up_rank_up_listener = self.set_up_rank_up_listener;
	...
	local taurox_subtype = 'unlocker_taurox_bst_taurox';
	local ghorros_subtype = 'unlocker_taurox_bst_ghorros';

	-- type, ancillary key, mission key, rank required, [optional] mission key if playing MPC
	local taurox_quests = {
		{ 'mission', 'unlocker_taurox_anc_weapon_rune_tortured_axe', 'unlocker_taurox_bst_taurox_dual_cleaver_stage1', 7 }
	};

  log('Setting up Taurox quest to unlock at rank', 7, 'with', 'unlocker_taurox_bst_taurox_dual_cleaver_stage1');
